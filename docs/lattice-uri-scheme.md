# Lattice official URI → on-chain `.reef` namespace

Product convention (not a browser protocol handler in the MVP):

| URI | Canonical FQDN on LatticeChain | Who may `registerNamespace` |
|-----|-------------------------------|-----------------------------|
| `lttc://governments.reef` | `governments.reef` | **Contract owner only** (slug is *reserved official*) |
| `lttc://lattice.reef` | `lattice.reef` | **Contract owner only** |
| `lttc://clipma.reef` | `clipma.reef` | Any caller with gas, if slug is not reserved |

Rules enforced on-chain (`LatticeChain.sol`):

- FQDN must match ASCII lowercase `[a-z0-9-]+\\.reef` (single label before `.reef`).
...
Verifiers map `lttc://<name>.reef` → `nameHash = keccak256(utf8(fqdn))` and read `namespaces(nameHash)`.
