...
describe('chain labelToBytes32', () => {
  it('hashes UTF-8 labels to deterministic bytes32', () => {
    expect(labelToBytes32('governments.reef')).toMatch(/^0x[0-9a-f]{64}$/i);
    expect(labelToBytes32('governments.reef')).toBe(labelToBytes32('governments.reef'));
  });
...
describe('assertValidPublicLatticeFqdn', () => {
  it('accepts lowercase label.reef', () => {
    expect(() => assertValidPublicLatticeFqdn('clipma.reef')).not.toThrow();
  });
