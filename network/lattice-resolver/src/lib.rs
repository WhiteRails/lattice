...
impl ResolverTable {
    pub fn new(
        entries: impl IntoIterator<Item = (String, Vec<IpAddr>)>,
        ttl: u32,
        ...
        for (name, addresses) in entries {
            if !lattice_net_core::profile::valid_service_fqdn(&name) || addresses.is_empty() {
                return Err(DnsError::Unsupported);
            }
            if table.insert(name, addresses).is_some() {
                return Err(DnsError::Unsupported);
            }
        }
        Ok(Self {
    ...
    pub fn answer(&self, query: &[u8]) -> Result<Vec<u8>, DnsError> {
        if query.len() < DNS_HEADER_BYTES || query.len() > MAX_DNS_MESSAGE_BYTES {
        ...
        if !name.ends_with(".lattice") && !name.ends_with(".coral") && !name.ends_with(".reef") {
...
mod tests {
    ...
    fn answer_addresses(message: &[u8]) -> Vec<IpAddr> {
        ...
        for _ in 0..answers {
            ...
            match (kind, data.len()) {
                (TYPE_A, 4) => result.push(IpAddr::V4(Ipv4Addr::new(
                    data[0], data[1], data[2], data[3],
                ))),
                (TYPE_AAAA, 16) => result.push(IpAddr::V6(Ipv6Addr::from(
    ...
    fn resolves_only_lattice_names() {
        let table = ResolverTable::new(
            [("echo.lattice".into(), vec!["10.64.0.2".parse().unwrap()])],
            30,
        )
        .unwrap();
        let answer = table.answer(&query("echo.lattice", TYPE_A)).unwrap();
        assert_eq!(
            answer_addresses(&answer),
            vec!["10.64.0.2".parse::<IpAddr>().unwrap()]
        );
    ...
    #[test]
    fn canonical_service_identity_resolves_to_the_service_virtual_ip() {
        let canonical = lattice_net_core::profile::canonical_service_fqdn(&"ab".repeat(32))
            .expect("a SHA-256 SPKI pin has a canonical hostname");
        let table = ResolverTable::new(
            [(canonical.clone(), vec!["10.64.0.2".parse().unwrap()])],
            30,
        )
        .unwrap();
        let answer = table.answer(&query(&canonical, TYPE_A)).unwrap();
        assert_eq!(
            answer_addresses(&answer),
            vec!["10.64.0.2".parse::<IpAddr>().unwrap()]
        );
